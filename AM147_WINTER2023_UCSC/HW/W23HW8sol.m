...
M = zeros(length(t)-1,length(t)); % initialize

M(:,1:length(t)-1) = -eye(length(t)-1);

M(:,2:end) = M(:,2:end) + eye(length(t)-1);

beta = [1; 10; 100];

line_style = {'-','-','-'}; line_color = {'r','c','g'};

b = [x_noisy; zeros(length(t)-1,1)];

for j=1:length(beta)
    
    % least squares estimate via A\b
    xhat(:,j) = [eye(length(t)); sqrt(beta(j))*M] \ b; 
