...
class LinkedList
{
	...
	public:
        ...
        Node* deleterec(int); // remove a node with int (if it exists), and return pointer to deleted node. This does not delete all nodes with the value.
        Node* deleterec(int, Node*); // recursive remove
		Node* deletekth(int, int);
		Node* deletekth(int, int, Node*);
		Node* deletelast(int); // remove a node with int (if it exists), and return pointer to deleted node. This does not delete all nodes with the value.
        Node* deletelast(int, Node*, Node*);
		Node* allbutfirst(int);
		Node* allbutfirst(int, int, Node*);
		bool ispalindrome();
