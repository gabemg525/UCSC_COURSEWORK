...
class NaiveBayes():
    ...
    def fit(self, train_hams:list, train_spams:list):
        ...
        for th_file in train_hams:
            self.num_train_hams+=1
            words_list = self.word_set(th_file)  
            appears_in_th_file = []
            # creates a list of words that appear in the file without duplicates
            [appears_in_th_file.append(w) for w in words_list if w not in appears_in_th_file]
            # sets the value to 1 if the word is not in the dict, increments if it is
            for key in appears_in_th_file:
                if key in self.word_counts_ham:
                    self.word_counts_ham[key]+=1
                else:
                    self.word_counts_ham[key]=1
        # repeats last block of code with spam files
        for ts_file in train_spams:
            self.num_train_spams+=1
            words_list = self.word_set(ts_file)
            appears_in_ts_file = []
            [appears_in_ts_file.append(w) for w in words_list if w not in appears_in_ts_file]
            for key in appears_in_ts_file:
                if key in self.word_counts_spam:
                    self.word_counts_spam[key]+=1
                else:
                    self.word_counts_spam[key]=1
    ...
    def predict(self, filename:str):
        ...
        """
        pass # TODO: Your code here (10-20 lines)
        # probability that the email is spam or ham
        ...
        file_words = [*set(self.word_set(filename))]
        balance_ham_spam = np.log(p_ham)-np.log(p_spam)
        for word in file_words:
            # probability that each word is a ham given it is ham
            if word in self.word_counts_ham:
                p_word_in_ham = (self.word_counts_ham[word]+1)/(self.num_train_hams+2)
            # laplace smoothing is needed for words that are not in the word_counts_ham dict
            else:
                p_word_in_ham = 1/(self.num_train_hams+2)
            # repeating if/else for spam
            if word in self.word_counts_spam:
                p_word_in_spam = (self.word_counts_spam[word]+1)/(self.num_train_spams+2)
            else:
                p_word_in_spam = 1/(self.num_train_spams+2)
            # the "balance" of spam or ham is adjusted by the log of the probability of spam or ham for each word
            balance_ham_spam += np.log(p_word_in_ham)-np.log(p_word_in_spam)
        if balance_ham_spam >= 0:
