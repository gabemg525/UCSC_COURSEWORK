...
void dg_serv_read(int sockfd, SA *pcliaddr, socklen_t clilen, int droppc, char rfpath[MAXLINE+1])
{
    int n;
    int i = 0;
    socklen_t len;
    ...
    unsigned int seq, seg_size, *total_p;
    ...
    while (1) {
        ...
        if(total_recvp[c_id]==0){
            ...
            sprintf(path[c_id], "./%s/%s", rfpath, curr_path);
            create_file_path(path[c_id]);
        ...
        if(recv_arr[c_id][seq]==false){
            ...
            // printf("to_write[i]: %s\nseg_sz: %d\n",to_write[c_id][seq],seg_sz_arr[c_id][seq]);
        }
...
int main(int argc, char **argv)
{
    ...
    int sockfd;
    if(argc != 4){
        fputs("usage: ./myserver <port> <droppc> <root folder path>\n",stderr);
        exit(EXIT_FAILURE);
    }
    ...
    dg_serv_read(sockfd, (SA *) &cliaddr, sizeof(cliaddr), atoi(argv[2]), argv[3]);
    exit(0);
    return 0;
}
