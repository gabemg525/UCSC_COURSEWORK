...
void BinHeap :: extractMax()
{
    ...
    int index = 1;
    while(index*2<=size){
        child1_index=index*2;
        child2_index=child1_index+1;
        cout<<"A[child1_index]: "<<A[child1_index]<<" A[child2_index]: "<<A[child2_index]<<endl;
        if(A[index]<max(A[child1_index],A[child2_index])){
            if(A[child1_index]>A[child2_index]){
                temp = A[index];
                A[index] = A[child1_index];
                A[child1_index] = temp;
                index = child1_index;
            }else{
                temp = A[index];
                A[index] = A[child2_index];
                A[child2_index] = temp;
                index = child2_index;
            }
        }else{
    ...
    // int index = 1;
    // int temp;
    // while(index*2<=size){    //if size < index*2, can't sink any further.
    //     int child_1 = index*2;
    //     int child_2 = child_1+1;
    //     if(A[index]<max(A[child_1],A[child_2])){
    //         if(A[child_1]==max(A[child_1],A[child_2])){
    //             temp=A[child_1];
    //             A[child_1]=A[index];
    //             A[index]=temp;
    //             index=child_1;
